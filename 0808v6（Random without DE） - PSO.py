...
def N_Rp_Op():
    ...
    lb_ABC = []
    ub_ABC = []
    for Num_of_Random_ in range(3,9):
        ...
        xopt_ABC,fopt_ABC = pso(Op_ABC, lb_ABC, ub_ABC)
        Result_Random_fun.append(fopt_ABC)
        Result_Random_x.append(xopt_ABC)
       
    Min_Result_Random_x = Result_Random_fun.index(min(Result_Random_fun))
    Adjust_x.append(Result_Random_x[Min_Result_Random_x])
    return Adjust_x
...
for i in range(len(Adjust_x[0])/2):
    lb_Ax.append(0.1)
    ub_Ax.append(len(Adjust_x[0])/2 - i - 0.1)

xopt_Ax,fopt_Ax = pso(Dis_PosC, lb_Ax, ub_Ax, maxiter=5)
print xopt_Ax
